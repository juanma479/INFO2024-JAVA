package com.info.app.projectapp.presentation.controller.proyecto;
...
import com.info.app.projectapp.presentation.errors.ErroresDto;
...
public class ProyectoController {
    ...
    public ResponseEntity<?> createProyecto(@RequestBody ProyectoCreateDto proyectoCreateDto) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body( proyectoCreatedDto.get());
    }
    ...
    public ResponseEntity<?> getProyectoById(@PathVariable("idProyecto") UUID idProyecto) {

        Optional<ProyectoDto> proyectoDto = proyectoService.getProyectoDtoById(idProyecto);

        if (proyectoDto.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ErroresDto(String.format("El proyecto(id=%s) no existe", idProyecto.toString()),
                            HttpStatus.NOT_FOUND.value(),"api/v1//proyecto/"+idProyecto.toString()));
        }

        return ResponseEntity.status(HttpStatus.OK).body(proyectoDto.get());
    }
}
